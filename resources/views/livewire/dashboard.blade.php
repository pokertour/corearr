...
<div class="space-y-8">
    <div class="flex flex-col md:flex-row md:items-center justify-between gap-4">
        <div>
            <h2 class="text-2xl font-bold tracking-tight text-zinc-900 dark:text-white">Command Center</h2>
            <p class="text-sm text-zinc-500">Aperçu en temps réel de votre infrastructure.</p>
        </div>
    ...
    @if($qbitConfigured)
        <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
            @foreach([
                ...
                ['label' => 'Vitesse Upload', 'value' => $this->formatSize($stats['up_speed']) . '/s', 'icon' => 'M8 7l4-4m0 0l4 4m-4-4v18', 'color' => 'teal'],
                ['label' => 'Volume Total', 'value' => $this->formatSize($stats['total_size']), 'icon' => 'M20 7l-8-4-8 4m16 0l-8 4m8-4v10l-8 4m0-10L4 7m8 4v10M4 7v10l8 4', 'color' => 'purple']
            ] as $stat)
                ...
                <div class="bg-white dark:bg-zinc-900 border border-zinc-200 dark:border-zinc-800 p-5 rounded-2xl shadow-sm hover:border-{{ $cfg['color'] }}-500/50 transition duration-300 relative group">
                    ...
                    @if(isset($arrStats[$id]['disk']))
                        <div class="mt-4 space-y-3">
                            ...
                            <div class="flex justify-between text-[10px] font-bold uppercase tracking-widest text-zinc-500">
                                <span>Libre: {{ $this->formatSize($arrStats[$id]['disk']['free']) }}</span>
                                <span>{{ round($percent) }}% Occupé</span>
                            </div>
                        </div>
                    @endif
                    ...
                    @if($id === 'radarr' && isset($arrStats['radarr']['movies']))
                         <div class="mt-4 pt-4 border-t border-zinc-100 dark:border-zinc-800 flex items-center justify-between">
                             <span class="text-[10px] font-black text-zinc-400 uppercase tracking-widest">Films</span>
                             <div class="flex items-center gap-2">
                    ...
                    @if($id === 'sonarr' && isset($arrStats['sonarr']['episodes']))
                         <div class="mt-4 pt-4 border-t border-zinc-100 dark:border-zinc-800 flex items-center justify-between">
                             <span class="text-[10px] font-black text-zinc-400 uppercase tracking-widest">Épisodes</span>
                             <div class="flex items-center gap-2">
    ...
    <div class="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        @if(!$qbitConfigured && !$arrConfigured)
            <div class="lg:col-span-3 flex flex-col items-center justify-center py-20 bg-zinc-50 dark:bg-zinc-900/50 rounded-3xl border-2 border-dashed border-zinc-200 dark:border-zinc-800">
                <p class="text-zinc-500 font-medium mb-4 text-center px-6">Aucun service n'est configuré. Visitez les paramètres pour commencer.</p>
                <a href="/settings" wire:navigate class="px-6 py-3 bg-zinc-900 dark:bg-zinc-100 text-white dark:text-zinc-900 font-bold rounded-2xl shadow-xl transition active:scale-95">Configurer la stack</a>
            </div>
        @endif
    </div>
</div>
