...
new #[Lazy] class extends Component {
    ...
    public function mount()
    {
        ...
        if ($this->isConfigured) {
            // Get settings to handle relative avatar URLs
            $settings = \App\Models\ServiceSetting::where('service_name', 'jellyseerr')->where('is_active', true)->first();
            $baseUrl = $settings ? rtrim($settings->base_url, '/') : '';
            ...
            $this->topUsers = collect($users)
                ->map(function ($user) use ($baseUrl) {
                    // Handle relative avatar paths from Jellyseerr
...
<div
    class="bg-white dark:bg-zinc-900 border border-zinc-200 dark:border-zinc-800 rounded-2xl shadow-sm overflow-hidden flex flex-col h-full">
    <div
        class="p-5 border-b border-zinc-100 dark:border-zinc-800 flex justify-between items-center bg-zinc-50/50 dark:bg-zinc-900/50">
            ...
            @if (count($topUsers) > 0)
                <ul class="space-y-3 mb-4">
                    @foreach ($topUsers as $index => $user)
                        <li
                            class="flex items-center gap-3 p-3 rounded-xl bg-zinc-50 dark:bg-zinc-800/50 border border-zinc-100 dark:border-zinc-800 relative overflow-hidden">
                            <!-- Rank Badge -->
                            <div
                                class="absolute left-0 top-0 bottom-0 w-1 flex flex-col items-center justify-center 
                                {{ $index === 0 ? 'bg-yellow-400' : ($index === 1 ? 'bg-zinc-400' : ($index === 2 ? 'bg-amber-600' : 'bg-zinc-200 dark:bg-zinc-700')) }}">
                            </div>

                            <div
                                class="w-10 h-10 rounded-full bg-zinc-200 dark:bg-zinc-700 font-bold flex items-center justify-center text-zinc-600 dark:text-zinc-300 ml-2 shadow-sm overflow-hidden shrink-0">
                                @if (!empty($user['avatar']))
                                    <img src="{{ $user['avatar'] }}" alt="Avatar" class="w-full h-full object-cover"
                                        onerror="this.style.display='none'" />
                                @endif
                                <span
                                    class="absolute">{{ substr($user['displayName'] ?? ($user['email'] ?? 'U'), 0, 1) }}</span>
                            </div>
                            ...
                            <div class="flex flex-col items-end">
                                <span
                                    class="text-lg font-black text-core-primary">{{ $user['requestCount'] ?? 0 }}</span>
                                <span
                                    class="text-[9px] uppercase font-bold tracking-widest text-zinc-400">Demandes</span>
                            </div>
                        </li>
                    @endforeach
                </ul>
