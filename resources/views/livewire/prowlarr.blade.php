...
new #[Livewire\Attributes\Layout('components.layouts.app')] #[Livewire\Attributes\Title('messages.indexers')] class extends Component {
    ...
    public function testIndexer(int $id)
    {
        ...
        $this->dispatch('toast', 
            message: $success ? __('messages.test_success') : __('messages.test_fail'),
...
<div class="space-y-6">
    <div class="flex flex-col md:flex-row md:items-center justify-between gap-4">
        <div>
            <h2 class="text-2xl font-bold tracking-tight text-zinc-900 dark:text-white">{{ __('messages.indexer_management') }}</h2>
    ...
    @if(!$isConfigured)
        <div class="flex flex-col items-center justify-center py-20 bg-white dark:bg-zinc-900 border border-dashed border-zinc-200 dark:border-zinc-800 rounded-3xl text-center">
            ...
            <h3 class="text-xl font-bold text-zinc-900 dark:text-zinc-100 mb-2">{{ __('messages.not_configured_title', ['service' => 'Prowlarr']) }}</h3>
            <p class="text-zinc-500 mb-6 max-w-sm">{{ __('messages.not_configured_subtitle', ['service' => 'Prowlarr', 'type' => __('messages.indexers')]) }}</p>
                ...
                <table class="w-full text-left border-collapse">
                    <thead>
                        <tr class="bg-zinc-50 dark:bg-zinc-800/50 border-b border-zinc-200 dark:border-zinc-800">
                            <th class="px-6 py-4 text-xs font-bold text-zinc-500 uppercase tracking-wider">{{ __('messages.indexer') }}</th>
                            <th class="px-6 py-4 text-xs font-bold text-zinc-500 uppercase tracking-wider">{{ __('messages.priority') }}</th>
                            <th class="px-6 py-4 text-xs font-bold text-zinc-500 uppercase tracking-wider">{{ __('messages.protocol') }}</th>
                            <th class="px-6 py-4 text-xs font-bold text-zinc-500 uppercase tracking-wider">{{ __('messages.status') }}</th>
                            <th class="px-6 py-4 text-xs font-bold text-zinc-500 uppercase tracking-wider">{{ __('messages.tags') }}</th>
                        ...
                        @forelse($indexers as $indexer)
                            <tr class="hover:bg-zinc-50 dark:hover:bg-zinc-800/40 transition group">
                                ...
                                <td class="px-6 py-4 text-right space-x-2">
                                    <button wire:click="toggleIndexer({{ $indexer['id'] }})" class="cursor-pointer p-1.5 text-zinc-400 hover:text-zinc-900 dark:hover:text-zinc-100 transition" title="{{ ($indexer['enable'] ?? false) ? __('messages.disabled') : __('messages.enabled') }}">
                                    ...
                                    <button wire:click="testIndexer({{ $indexer['id'] }})" class="cursor-pointer p-1.5 text-zinc-400 hover:text-green-500 transition" title="{{ __('messages.test_all') }}">
                                        <svg class="w-4 h-4" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 12l2 2 4-4m6 2a9 9 0 11-18 0 9 9 0 0118 0z"/></svg>
                                    </button>
                                    <button wire:click="deleteIndexer({{ $indexer['id'] }})" 
                                            wire:confirm="{{ __('messages.delete_confirm') }}"
                                            class="cursor-pointer p-1.5 text-zinc-400 hover:text-red-500 transition" title="{{ __('messages.delete') }}">
