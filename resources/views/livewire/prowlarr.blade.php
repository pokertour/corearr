...
new #[Livewire\Attributes\Layout('components.layouts.app')] #[Livewire\Attributes\Title('messages.indexers')] class extends Component {
    ...
    public function testIndexer(int $id)
    {
        ...
        $this->dispatch('toast', message: $success ? __('messages.test_success') : __('messages.test_fail'), type: $success ? 'success' : 'error');
    ...
    public function deleteIndexer(int $id)
    {
        ...
        $this->dispatch('toast', message: __('messages.indexer_deleted'), type: 'success');
        $this->loadIndexers();
    }
    ...
    public function toggleIndexer(int $id)
    {
        ...
        $this->dispatch('toast', message: $indexer['enable'] ? __('messages.indexer_enabled') : __('messages.indexer_disabled'), type: 'success');
        $this->loadIndexers();
    }
    ...
    public function testAll()
    {
        ...
        $this->dispatch('toast', message: __('messages.bg_test_started'), type: 'info');
        $this->loading = false;
    }
    ...
    public function updatePriority(int $id, int $priority)
    {
        ...
        if ($success) {
            $this->dispatch('toast', message: __('messages.priority_updated'), type: 'success');
    ...
    <div class="flex flex-col md:flex-row md:items-center justify-between gap-4">
        <div>
            <h2 class="text-2xl font-bold tracking-tight text-zinc-900 dark:text-white">
                {{ __('messages.indexer_management') }}</h2>
            <p class="text-sm text-zinc-500">{{ __('messages.indexer_subtitle') }}</p>
        </div>

        <div class="flex items-center gap-3">
            <button wire:click="testAll"
                class="cursor-pointer px-4 py-2 bg-white dark:bg-zinc-900 border border-zinc-200 dark:border-zinc-800 rounded-xl text-sm font-bold text-zinc-700 dark:text-zinc-300 hover:bg-zinc-50 dark:hover:bg-zinc-800 transition flex items-center gap-2">
                <svg class="w-4 h-4" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                        d="M9 12l2 2 4-4m6 2a9 9 0 11-18 0 9 9 0 0118 0z" />
                </svg>
                {{ __('messages.test_all') }}
            </button>
            <button wire:click="loadIndexers"
                class="cursor-pointer p-2 bg-core-primary text-white rounded-xl hover:bg-core-primary/90 transition shadow-lg shadow-core-primary/20">
    ...
    @if (!$isConfigured)
        <div
            ...
            <h3 class="text-xl font-bold text-zinc-900 dark:text-zinc-100 mb-2">
                {{ __('messages.not_configured_title', ['service' => 'Prowlarr']) }}</h3>
            <p class="text-zinc-500 mb-6 max-w-sm">
                {{ __('messages.not_configured_subtitle', ['service' => 'Prowlarr', 'type' => __('messages.indexers')]) }}
            </p>
                    ...
                    <thead>
                        <tr class="bg-zinc-50 dark:bg-zinc-800/50 border-b border-zinc-200 dark:border-zinc-800">
                            <th class="px-6 py-4 text-xs font-bold text-zinc-500 uppercase tracking-wider">
                                {{ __('messages.indexer') }}</th>
                            <th class="px-6 py-4 text-xs font-bold text-zinc-500 uppercase tracking-wider">
                                {{ __('messages.priority') }}</th>
                            <th class="px-6 py-4 text-xs font-bold text-zinc-500 uppercase tracking-wider">
                                {{ __('messages.protocol') }}</th>
                            <th class="px-6 py-4 text-xs font-bold text-zinc-500 uppercase tracking-wider">
                        ...
                        @forelse($indexers as $indexer)
                            <tr class="hover:bg-zinc-50 dark:hover:bg-zinc-800/40 transition group">
                                <td class="px-6 py-4">
                                    <div class="flex items-center gap-3">
                                        <div
                                            class="w-8 h-8 rounded-lg bg-zinc-100 dark:bg-zinc-800 flex items-center justify-center font-bold text-zinc-400 text-[10px]">
                                        ...
                                        <div>
                                            ...
                                            <div class="text-[10px] text-zinc-500">
                                                {{ parse_url($indexer['fields'][0]['value'] ?? '', PHP_URL_HOST) ?? 'Indexer' }}
                                            </div>
                                        </div>
                                    </div>
                                </td>
                                <td class="px-6 py-4">
                                    <div class="flex items-center gap-2">
                                        <input type="number" value="{{ $indexer['priority'] ?? 25 }}"
                                ...
                                <td class="px-6 py-4">
                                    <span
                                        class="px-2 py-0.5 text-[10px] font-bold rounded bg-zinc-100 dark:bg-zinc-800 text-zinc-600 dark:text-zinc-400 border border-zinc-200 dark:border-zinc-700 uppercase">
                                ...
                                <td class="px-6 py-4">
                                    <div class="flex items-center gap-2">
                                        <div
                                            class="w-2 h-2 rounded-full {{ $indexer['enable'] ?? false ? 'bg-green-500' : 'bg-zinc-400' }}">
                                        </div>
                                        <span
                                            class="text-xs font-medium {{ $indexer['enable'] ?? false ? 'text-zinc-700 dark:text-zinc-300' : 'text-zinc-400' }}">
                                ...
                                <td class="px-6 py-4">
                                    <div class="flex flex-wrap gap-1">
                                        @foreach ($indexer['tags'] ?? [] as $tag)
                                            <span
                                                class="px-1.5 py-0.5 text-[9px] font-bold bg-blue-500/10 text-blue-500 rounded">{{ $tag }}</span>
                                        @endforeach
                                    </div>
                                </td>
                                <td class="px-6 py-4 text-right space-x-2">
                                    <button wire:click="toggleIndexer({{ $indexer['id'] }})"
                                        class="cursor-pointer p-1.5 text-zinc-400 hover:text-zinc-900 dark:hover:text-zinc-100 transition"
                                        title="{{ $indexer['enable'] ?? false ? __('messages.disabled') : __('messages.enabled') }}">
                                    ...
                                    <button wire:click="testIndexer({{ $indexer['id'] }})"
                                        class="cursor-pointer p-1.5 text-zinc-400 hover:text-green-500 transition"
                                        title="{{ __('messages.test_all') }}">
                                        <svg class="w-4 h-4" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                                                d="M9 12l2 2 4-4m6 2a9 9 0 11-18 0 9 9 0 0118 0z" />
                                        </svg>
                                    ...
                                    <button wire:click="deleteIndexer({{ $indexer['id'] }})"
                                        ...
                                        class="cursor-pointer p-1.5 text-zinc-400 hover:text-red-500 transition"
