...
new #[Layout('components.layouts.app')] #[Title('Paramètres')] class extends Component {
    ...
    public function save(string $serviceName)
    {
        ...
        $this->dispatch('notify', title: "Paramètres $serviceName sauvegardés.", type: 'success');
    }
    ...
    public function test(string $serviceName, MediaStackService $mediaService)
    {
        ...
        if (empty($params['base_url'])) {
            $this->dispatch('notify', title: 'URL manquante', message: 'Veuillez saisir une URL de base.', type: 'error');
            return;
        }
        ...
        if ($result['success']) {
            $this->dispatch('notify', title: 'Connexion réussie !', message: $result['message'], type: 'success');
        } else {
            $this->dispatch('notify', title: 'Échec du test', message: $result['message'], type: 'error');
        }
    }
};
...
<div class="space-y-6">
    <div>
        <h2 class="text-2xl font-bold tracking-tight text-zinc-900 dark:text-white">Paramètres des Services</h2>
        <p class="text-sm text-zinc-500">Configurez vos accès aux APIs Sonarr, Radarr, Prowlarr et qBittorrent.</p>
    </div>
        ...
        @foreach(['sonarr' => 'Sonarr', 'radarr' => 'Radarr', 'qbittorrent' => 'qBittorrent', 'prowlarr' => 'Prowlarr'] as $key => $label)
            <div class="bg-white dark:bg-zinc-900 border border-zinc-200 dark:border-zinc-800 rounded-2xl p-6 shadow-sm">
                ...
                <form wire:submit="save('{{ $key }}')" class="space-y-4">
                    <div>
                        <label class="block text-sm font-medium border-core-primary text-zinc-700 dark:text-zinc-300 mb-1">URL de base</label>
                    ...
                    @if($key === 'qbittorrent')
                        <div class="grid grid-cols-2 gap-4">
                            <div>
                                <label class="block text-sm font-medium text-zinc-700 dark:text-zinc-300 mb-1">Utilisateur</label>
                            ...
                            <div>
                                <label class="block text-sm font-medium text-zinc-700 dark:text-zinc-300 mb-1">Mot de passe</label>
                    ...
                    @else
                        <div>
                            <label class="block text-sm font-medium text-zinc-700 dark:text-zinc-300 mb-1">Clé API / Token</label>
                    ...
                    <div class="pt-2 flex items-center gap-3">
                        ...
                        <button type="button" 
                            ...
                            <span wire:loading.remove wire:target="test('{{ $key }}')">Tester la connexion</span>
                            <span wire:loading wire:target="test('{{ $key }}')">Test en cours...</span>
                        </button>
                    </div>
                </form>
            </div>
