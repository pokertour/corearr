...
class DeleteMediaJob implements ShouldQueue
{
    ...
    public function handle(
        ...
        Log::debug("DeleteMediaJob Started: {$this->title} (JS Req: {$this->requestId}, JS Media: {$this->jellyseerrMediaId}, Arr ID: {$this->externalId}, TMDB: {$this->tmdbId})");
        ...
        try {
            ...
            if ($this->tmdbId > 0) {
                Log::debug("DeleteMediaJob: Searching for all IDs by TMDB {$this->tmdbId}");
                $foundIds = $arrService->findMediaIdsByTmdbId($this->service, $this->tmdbId);
                if (! empty($foundIds)) {
                    Log::debug('DeleteMediaJob: Found IDs in '.$this->service.': '.implode(', ', $foundIds));
            ...
            if (! empty($effectiveIds)) {
                foreach ($effectiveIds as $id) {
                    Log::debug("DeleteMediaJob: Deleting from {$this->service} (ID: {$id})");
                    $success = $arrService->deleteMedia($this->service, $id, true);
                    Log::debug('DeleteMediaJob: '.$this->service." ID {$id} deletion result: ".($success ? 'SUCCESS' : 'FAILED'));
                    if (! $success) {
                        $arrDeleted = false;
                    }
                }
                ...
                if ($arrDeleted) {
                    ...
                    if ($stillThere) {
                        ...
                        Log::debug("DeleteMediaJob: Post-delete verification PASSED! All matching items gone from {$this->service}");
                    }
                }
            } else {
                Log::debug("DeleteMediaJob: No Arr IDs found for {$this->title}. Proceeding with Jellyseerr cleanup.");
            }

            if ($arrDeleted) {
                // 2. Delete from Jellyseerr
                if ($this->jellyseerrMediaId) {
                    Log::debug("DeleteMediaJob: Deleting Jellyseerr Media object {$this->jellyseerrMediaId}");
                    $jsDeleted = $jellyseerr->deleteMedia($this->jellyseerrMediaId);
                    Log::debug('DeleteMediaJob: Jellyseerr Media deletion result: '.($jsDeleted ? 'SUCCESS' : 'FAILED'));
                } else {
                    Log::debug("DeleteMediaJob: No Media ID, deleting Jellyseerr request {$this->requestId}");
                    $jsDeleted = $jellyseerr->deleteRequest($this->requestId);
                    Log::debug('DeleteMediaJob: Jellyseerr Request deletion result: '.($jsDeleted ? 'SUCCESS' : 'FAILED'));
                }
            } else {
